  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = isModelVisible
        ? `${REACT_APP_API_URL}/vehicles/${userData?._id}` // For edit mode, include the user ID in the URL
        : `${REACT_APP_API_URL}/vehicles`; // For registration, no user ID is needed
      ...
      if (response.ok) {
        ...
        localStorage.setItem("token", data?.vehicle?.jwtToken);
        localStorage.setItem("userObj", JSON.stringify(data?.vehicle));
