...
const TransportEmissions = (tab) => {
    ...
    useEffect(() => {
        const fetchTransportEmissions = async () => {
            try {
                const response = await fetch(`${REACT_APP_API_URL}/transport-emissions/${user._id}`, {
                    method: "GET",
                    headers: {
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await fetch(`${REACT_APP_API_URL}/transport-emissions/${deleteRecordId}`, {
                method: "DELETE",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${JWT_ADMIN_SECRET}`,
    ...
    const handleInputChange = (e, field) => {
        setCurrentRecord((prev) => ({ ...prev, [field]: e.target.value, userId: `${user._id}`, }));
    };

    // Submit new or updated record
    const handleSubmit = async (e, isUpdate = isEdit) => {
        e.preventDefault();
        console.log(isUpdate ? "Updating record..." : "Adding new record...", currentRecord);
        ...
        try {
            ...
            console.log(
                `Transport Records record ${isUpdate ? "updated" : "added"} successfully!`,
                await response.json()
            );
            window.location.reload();
        } catch (error) {
        ...
        <>
                                ...
                                {transportRecords.length > 0 ? (
                                    transportRecords.map((record, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <button className="btn btn-info btn-sm me-2" onClick={() => handleEdit(record)}>Edit</button>
            ...
            <Modal show={showModal} onHide={() => setShowModal(false)}>
                <Modal.Header closeButton>
                    <Modal.Title>{currentRecord._id ? "Edit" : "Add"} Transport Emission Record</Modal.Title>
                </Modal.Header>
                <Form onSubmit={handleSubmit}>
                    <Modal.Body>
                        <Form.Group className="mb-3">
                            <Form.Label>Month</Form.Label>
                            <Form.Control as="select" value={currentRecord.month} onChange={(e) => handleInputChange(e, "month")}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control type="number" value={currentRecord.year} onChange={(e) => handleInputChange(e, "year")} placeholder="Enter Year" />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            <Form.Label>Transport Mode</Form.Label>
                            <Form.Control as="select" value={currentRecord.transportMode} onChange={(e) => handleInputChange(e, "transportMode")}>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control type="number" value={currentRecord.distance} onChange={(e) => handleInputChange(e, "distance")} placeholder="Enter Distance" />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            <Form.Label>Weight (tons)</Form.Label>
                            <Form.Control type="number" value={currentRecord.weight} onChange={(e) => handleInputChange(e, "weight")} placeholder="Enter Weight" />
                        </Form.Group>
                        <Form.Group className="mb-3">
                            <Form.Label>Emission Factor (kg CO₂/ton-km)</Form.Label>
                            <Form.Control type="number" value={currentRecord.emissionFactor} onChange={(e) => handleInputChange(e, "emissionFactor")} placeholder="Enter Emission Factor" />
                        </Form.Group>
                    </Modal.Body>
                    <Modal.Footer>
                        <Button variant="secondary" onClick={() => setShowModal(false)}>Cancel</Button>
                        <Button variant="success" type="submit">{isEdit ? 'Update' : 'Save'} </Button>
                    </Modal.Footer>
                </Form>
            </Modal>
